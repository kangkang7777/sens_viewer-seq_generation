...
#define M_SENSOR_DATA_VERSION 4
		...
		void saveToImages(const std::string& outputFolder, const std::string& basename = "frame-") const {
...

			{
				...
				const std::string& metaDataColorIntrinsics = "colorIntrinsics.txt";
				std::ofstream outMetaDataColorIntrinsics(outputFolder + "/" + metaDataColorIntrinsics);
				for (unsigned int i = 0; i < 16; i++)
				{
					outMetaDataColorIntrinsics << m_calibrationColor.m_intrinsic.matrix[i] << " ";	
					if ((i + 1) % 4 ==0) outMetaDataColorIntrinsics << "\n";
				}
				outMetaDataColorIntrinsics.close();

				const std::string& metaDataDepthIntrinsics = "depthIntrinsics.txt";
				std::ofstream outMetaDataDepthIntrinsics(outputFolder + "/" + metaDataDepthIntrinsics);
				for (unsigned int i = 0; i < 16; i++)
				{
					outMetaDataDepthIntrinsics << m_calibrationDepth.m_intrinsic.matrix[i] << " ";
					if ((i+1) % 4 == 0) outMetaDataDepthIntrinsics << "\n";
				}
				outMetaDataDepthIntrinsics.close();
			}
			...
			for (size_t i = 0; i < m_frames.size(); i++) {
				...
				//color data
				const bool writeColorData = false;
				if (writeColorData)
				{
				...
				//depth data
				const bool writeDepthData = false;
				if (writeDepthData)
				{
